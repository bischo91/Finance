...
def which_bank(file_name_input):
    ...
    if 'CHASE' in file_name_input.upper() and '3225' in file_name_input:
        ...
        return 'Chase Credit'
    elif 'activity' in file_name_input and 'CHASE' not in file_name_input.upper() and 'DISCOVER' not in file_name_input.upper():
        return 'Amex'
    else:
        return 'Unknown'
...
def bank_condition(bank_name, desc, amt):
    ...
    if bank_name == 'Chase':
        ...
        cond = ('DISCOVER' not in desc and 'ROBINHOOD' not in desc and 'MATTRIX' not in desc and 'Manual' not in desc and 'CHASE CREDIT' not in desc.upper() and 'Online Transfer' not in desc)
        ...
        # for bank name amex, spending will not count if it's Mobile Payment
        cond = ('MOBILE PAYMENT' not in desc) and ('ONLINE PAYMENT' not in desc)
    return cond
...
def find_category_discover(bank_name, desc, cat, amt):
    ...
    if bank_name == 'Discover':
        if cat == 'Supermarkets':
            ...
            cat_return = cat
    return cat_return
def find_category_amex(bank_name, desc, cat, amt):
    if bank_name == 'Amex':
        if 'Fuel' in cat:
            if float(amt) < 10:
                cat_return = 'BC'
            ...
            cat_return = 'Grocery'
        elif 'Merchandise' in cat and 'Groceries' not in cat:
            cat_return = 'Merchandise'
        elif 'GAINESVILLE HEALTH' in desc.upper():
            cat_return = 'Gym'
        else:
            cat_return = cat
    return cat_return
...
def spending(path_abs, month_selected, year_selected, only_saved_data):
    ...
    for file_name_raw in file_list:
        ...
        if os.path.splitext(file_name_raw)[1] == '.xlsx':
            ...
            elif bank_name == 'Chase' or bank_name == 'Discover' or bank_name == 'Amex':
                # if data is from raw Excel (Chase/Discover/Amex)
                # bank define
                i = 2
                if bank_name == 'Amex':
                    i = 8
                while is_float_try(ws['D' + str(i)].value):
                    if bank_name == 'Discover' or bank_name == 'Amex':
                        date_spent = ws['A' + str(i)].value
                    # assuming Chase if not Discover nor Amex
                    ...
                    if int(date_spent[:2]) == month_selected and int(date_spent[-4:]) == year_selected:
                        if 'Payment to Chase' not in ws['C' + str(i)].value:
                            ...
                            if bank_name == 'Discover' or bank_name == 'Chase':
                                desc = ws['C' + str(i)].value
                                amt = ws['D' + str(i)].value
                            elif bank_name == 'Amex':
                                desc = ws['B' + str(i)].value
                                amt = ws['E' + str(i)].value
                                cat = ws['M' + str(i)].value

                            if bank_condition(bank_name, desc, amt):
                                # for Discover
                                if bank_name == 'Discover':
                                    cat = [find_category_discover(bank_name, desc, ws['E' + str(i)].value,  amt)]
                                # for Chase
                                elif bank_name == 'Chase':
                                    amt = -float(ws['D' + str(i)].value)
                                    cat = [find_category_chase(bank_name, desc, amt)]
                                elif bank_name == 'Amex':
                                    cat = [find_category_amex(bank_name, desc, cat, amt)]
...
class Ui_Dialog(QWidget):
    ...
    def cat_update(self):
        ...
        self.radiobutton_bank_6.setVisible(True)
        self.checkbox_all = []
    ...
    def setupUi(self, Dialog):
        ...
        self.radiobutton_bank_6 = QtWidgets.QRadioButton(self.frame_8)
        self.radiobutton_bank_6.setGeometry(QtCore.QRect(0, 210, 70, 20))
        self.radiobutton_bank_6.setText("Amex")
        self.radiobutton_bank_6.setVisible(False)
        self.radiobutton_bank_6.clicked.connect(self.cat_selected_cal)
        # Frame 4
